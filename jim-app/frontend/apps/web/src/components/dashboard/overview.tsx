...
function ContratStatusBadge({ statut }: { statut: string }) {
  ...
  const c = config[statut] ?? { bg: 'bg-gray-100', text: 'text-gray-500', label: statut };
  return (
    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase ${c.bg} ${c.text}`}>
      {c.label}
    </span>
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight">
            Bonjour{profileName ? `, ${profileName}` : ''}
          </h2>
        ...
        <div className="bg-gray-100 px-4 py-2 rounded-xl flex items-center gap-3 border border-gray-200/50">
          <Coins className="text-[#ff7c5c]" size={20} />
          <div>
            <p className="text-[10px] uppercase font-bold text-gray-400">Versements {currentMonth}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
        ...
        <div className="group bg-white p-6 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 hover:border-[#ff7c5c]/30 transition-all cursor-pointer relative overflow-hidden">
          <CalendarClock
            className="absolute -top-4 -right-4 text-gray-900 opacity-5 group-hover:opacity-10 transition-opacity"
            size={120}
          />
        ...
        <div className="group bg-white p-6 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 hover:border-blue-300/50 transition-all cursor-pointer relative overflow-hidden">
          <Inbox
            className="absolute -top-4 -right-4 text-gray-900 opacity-5 group-hover:opacity-10 transition-opacity"
            size={120}
          />
        ...
        <section className="lg:col-span-8 space-y-6">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-1">
              <button type="button" aria-label="Semaine precedente" className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 transition-colors">
                <ChevronLeft size={16} />
              </button>
              <button type="button" aria-label="Semaine suivante" className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-400 transition-colors">
              ...
              {upcomingContrats.map((contrat, index) => {
                ...
                return (
                  <div key={contrat.id} className={`relative pl-8 ${index < upcomingContrats.length - 1 ? 'before:absolute before:left-[11px] before:top-0 before:bottom-0 before:w-[2px] before:bg-[#ff7c5c]/20' : 'before:absolute before:left-[11px] before:top-0 before:h-1/2 before:w-[2px] before:bg-[#ff7c5c]/20'}`}>
                    ...
                    <div className={`absolute left-0 top-1 w-6 h-6 rounded-full border-4 border-white shadow-sm z-10 ${isFirst ? 'bg-[#ff7c5c]' : 'bg-gray-200'}`} />

                    {/* Carte */}
                    <div className={`p-5 rounded-2xl border flex items-center gap-6 transition-all ${
                      isFirst
                        ? 'bg-[#ff7c5c] text-white shadow-lg border-[#ff7c5c]'
                        : contrat.statut === 'confirme'
                          ? 'bg-white border-gray-100 hover:border-[#ff7c5c]/20'
                          : 'bg-gray-50/50 border-dashed border-gray-200 opacity-80'
                    }`}>
                      {/* Colonne date */}
                      {shortDate && (
                        <div className={`text-center pr-6 ${isFirst ? 'border-r border-white/20' : 'border-r border-gray-100'}`}>
                          <p className={`text-[10px] font-black uppercase ${isFirst ? 'text-white/80' : 'text-gray-400'}`}>{shortDate.dayLabel}</p>
                          <p className={`text-2xl font-black leading-none ${isFirst ? 'text-white' : 'text-gray-900'}`}>{shortDate.dayNumber}</p>
                        </div>
                      )}

                      {/* Contenu */}
                      <div className="flex-1 min-w-0">
                        <p className={`text-[10px] font-bold uppercase ${isFirst ? 'text-white/70' : 'text-gray-400'}`}>
                        ...
                        <h4 className={`font-extrabold text-lg truncate ${isFirst ? 'text-white' : 'text-gray-900'}`}>{remplacantName}</h4>
                        {donnees.adresse_cabinet && (
                          <div className={`flex items-center gap-1 mt-0.5 text-xs ${isFirst ? 'text-white/70' : 'text-gray-500'}`}>
        ...
        <aside className="lg:col-span-4 space-y-8">
          ...
          <div className="space-y-3">
            ...
            {candidaturesEnAttente > 0 ? (
              <div className="bg-red-50 border border-red-100 p-5 rounded-2xl space-y-4 shadow-sm">
                ...
                <button
                  type="button"
                  className="w-full py-3 bg-red-600 text-white text-sm font-bold rounded-xl hover:bg-red-700 transition-colors shadow-md shadow-red-200"
                >
              ...
              <div className="bg-green-50 border border-green-100 p-5 rounded-2xl shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-sm font-bold text-green-900">Tout est en ordre</p>
                </div>
              ...
              <div className="space-y-4">
                {paiementsCeMois.slice(0, 5).map((p) => (
                  <div key={p.id} className="flex items-center gap-3">
                    ...
                    <Receipt className="text-gray-300 flex-shrink-0" size={18} />
                  </div>
                ))}
              </div>
            )}
          ...
          <Link
            href="/publier"
            className="block bg-[#ff7c5c] p-5 rounded-2xl text-white shadow-lg hover:brightness-105 transition-all"
          >
