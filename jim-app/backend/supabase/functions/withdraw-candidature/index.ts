// Edge Function withdraw-candidature — Epic 5, Story 5.8
// Handler ≤ 40 lignes — délègue à _shared/candidature.service.ts
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2';
import { withdrawCandidatureSchema } from '../_shared/candidature.schema.deno.ts';
import { withdrawCandidature } from '../_shared/candidature.service.ts';
...
Deno.serve(async (req: Request) => {
  ...
  const parsed = withdrawCandidatureSchema.safeParse(body);
  if (!parsed.success) {
    return Response.json({ error: { code: 'VALIDATION_ERROR', message: parsed.error.issues[0]?.message } }, { status: 422, headers: corsHeaders });
  }

  const result = await withdrawCandidature(supabase, user.id, parsed.data.candidature_id);
  if (result.error) {
